...
if (!$reservaAntes || $reservaAntes['status'] !== 'CHECKIN_REALIZADO') {
    $_SESSION['erro'] = 'Reserva não pode ser check-out';
    header('Location: reserva_detalhes.php?id=' . $reservaId);
    exit;
}

// ============================================
// MODIFICAÇÃO: ATUALIZAR STATUS DO QUARTO PARA "DISPONIVEL"
// Adicionado em 2025-12-30 para sincronização automática
// ============================================
// Quando o check-out é realizado, o quarto deve voltar a ficar DISPONIVEL
atualizarStatusQuarto($reservaAntes['quarto_id'], 'DISPONIVEL');
// ============================================

// Atualizar reserva
$stmt = $db->prepare("UPDATE reservas SET status = 'CHECKOUT_REALIZADO' WHERE id = ?");
$stmt->execute([$reservaId]);
...
// Gerar PDF de check-out (opcional - pode ser gerado manualmente depois)
// Link disponível em reserva_detalhes.php

$_SESSION['sucesso'] = 'Check-out realizado com sucesso!';
header('Location: reserva_detalhes.php?id=' . $reservaId);
exit;